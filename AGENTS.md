...
- The theme guard (`Plugin::is_ollie_active()`, i.e. `get_template() === 'ollie'`) is **silent by design** — Ollie Pro is a hard dependency (`Requires Plugins`) and already reports a wrong theme. `get_template()`, not `get_stylesheet()`, so Ollie child themes pass.
- The guard gates **only the header feature**. The Updater is wired before it and runs under every theme: GitHub is the sole distribution channel, so a site parked on another theme would otherwise never learn a new version exists. Owner decision, 2026-07-17 — staying updated outranks staying inert. Do not "tidy" the Updater back behind the guard.
